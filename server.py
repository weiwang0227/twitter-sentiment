...
def add_color(tweets):
    ...
    for t in tweets:
        score = t['score']
        t['color'] = colors[int(round((score - -1.0) * 100 / 2))]
    return tweets

@app.route("/<name>")
def tweets(name):
    "Display the tweets for a screen name color-coded by sentiment score"
    user_tweets = fetch_tweets(api, name)
    add_color(user_tweets['tweets'])
    score_vector = [ each_tweet['score'] for each_tweet in user_tweets['tweets']]
    median_score = median(score_vector)
...
def following(name):
    ...
    """
    user_list = fetch_following(api, name)
    user_list.sort(key = lambda dic:dic['followers'], reverse = True)
